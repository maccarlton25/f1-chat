...
import { F1ToolVisualization, hasF1Visualization } from "./f1/tool-visualization";
  ...
  switch (part.type) {
    ...
    case "dynamic-tool": {
      const hasViz =
        ...
        part.output &&
        hasF1Visualization(part.toolName);
        ...
        if (showToolCalls) {
          return (
            <div className="flex flex-col gap-2 w-full">
              ...
              <F1ToolVisualization toolName={part.toolName} output={part.output} />
            </div>
          );
        }
        return <F1ToolVisualization toolName={part.toolName} output={part.output} />;
      }

      if (!showToolCalls) {
        if (part.state === "output-available" || part.state === "approval-requested" || part.state === "approval-responded") {
          return null;
        }
        return (
          <div className="flex items-center gap-2 text-xs text-muted-foreground py-1">
            <span className="inline-flex size-3 animate-spin rounded-full border-2 border-muted-foreground/30 border-t-muted-foreground" />
            Looking up F1 data…
          </div>
        );
      }
